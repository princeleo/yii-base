...
var App = function() {
    ...
    var uiInit = function() {
        ...
        // Initialize Tooltips
        $('[data-toggle="tooltip"], .enable-tooltip').tooltip({trigger: "manual",container: 'body', animation: false})
        ...
        //图片旋转
        $('[data-toggle="lightbox-image"],.gallery-link').click(function() {
          setTimeout(function(){
            var num = 0;
            $(".mfp-rotate").click(function(){
              console.log("123");
              num ++;
              $(".mfp-content .mfp-img").rotate(90*num);
            });
          },300)
        })
        ...
        $('.input-datepicker, .input-daterange').click(function(){
        ...
        $('.input-single').daterangepicker(congif_daterangepicker.single);

        //选择天数
        $('.one-day-input').daterangepicker(congif_daterangepicker.oneTimePicker)
            .on('apply.daterangepicker', function(ev, picker) {
                var  textTime = '';
                var len = $('.tag').length;
                var one_time = $(this.element).attr('data-start-time-input');
                $(this).val(picker.startDate.format('YYYY-MM-DD'));
                if($(this).val()!=textTime){
                    textTime = $(this).val();
                    for (var i = 0 ; i<len ;i++ ) {
                        if (textTime === $('.tag').eq(i).children('span').text()) {
                          return false;
                        }
                    }
                    $('#days1').append('<span class="tag"><span>'+textTime +'</span><a title="删除">x</a><input type="hidden" name="days[]" class="day-tag-rules" value='+textTime+'></span>');
                }
                $("#days1 .tag a").click(function(event){
        ...
        //开始时间
        $('.startTimepicker').daterangepicker(congif_daterangepicker.startTimePicker)
            .on('apply.daterangepicker', function(ev, picker) {
        ...
        //结束时间
        $('.endTimepicker').daterangepicker(congif_daterangepicker.endTimePicker)
            .on('apply.daterangepicker', function(ev, picker) {
        ...
        $('.input-singletimepicker').daterangepicker(congif_daterangepicker.singleTimePicker)
            .on('apply.daterangepicker', function(ev, picker) {
                $(this).val(picker.startDate.format('YYYY-MM-DD HH:mm:ss'));
            })
            .on('cancel.daterangepicker', function(ev, picker) {
        ...
        //不允许选择过去时间
        $('.currentTimepicker').daterangepicker(congif_daterangepicker.currentTimePicker)
            .on('apply.daterangepicker', function(ev, picker) {
                $(this).val(picker.startDate.format('YYYY-MM-DD'));
            })
            .on('cancel.daterangepicker', function(ev, picker) {
        ...
        $('.input-double').attr('readonly',true);//不允许手动输入
        $('.input-double').daterangepicker(congif_daterangepicker.double,function(start, end, label){
            var start_input = $(this.element).attr('data-start-time-input'),
                end_input = $(this.element).attr('data-end-time-input');
            if(start_input){
                $('input[name="'+start_input+'"').val(start.format('YYYY-MM-DD HH:mm:ss'));
            }
            if(end_input){
                $('input[name="'+end_input+'"').val(end.format('YYYY-MM-DD HH:mm:ss'));
            }
        }).on('cancel.daterangepicker',function(ev,picker){
                $(picker.element).val('');
                var start_input = picker.element.attr('data-start-time-input'),
                    end_input = picker.element.attr('data-end-time-input');
                if(start_input){
      ...
      $('.finish-date').daterangepicker(congif_daterangepicker.finishDate,function(start, end, label){
        var start_input = $(this.element).attr('data-start-time-input'),
            end_input = $(this.element).attr('data-end-time-input');
        if(start_input){
          $('input[name="'+start_input+'"').val(start.format('YYYY-MM-DD HH:mm:ss'));
        ...
        // input double default value
        (function($){
            var inputDateArray;
            inputDateArray =[$('.input-double'),$('.finish-date')];
            inputDateArray.forEach(function(inputDate){
            inputDate.each(function() {
                var dateStartAttr, dateEndAttr, inputHideStart, inputHideEnd;
                var _this = $(this);
                dateStartAttr = _this.attr('data-start-time-input');
                dateEndAttr = _this.attr('data-end-time-input');
                inputHideStart = $('input[name='+ dateStartAttr +']').val();
                inputHideEnd = $('input[name='+ dateEndAttr +']').val();
                console.log(inputHideStart, inputHideEnd);
                if (inputHideStart && inputHideEnd) {
                    _this.val(inputHideStart.split(' ')[0] + ' 至 ' + inputHideEnd.split(' ')[0]);
                }
            });
          });
        })(jQuery);
...
$('#sidebar').on('mousewheel', function(event) {
    // 一些数据
